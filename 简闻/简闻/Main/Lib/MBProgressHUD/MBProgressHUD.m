...
- (void)layoutSubviews {
    ...
    NSAttributedString *attrib = label.attributedText; // 方法过期，自己加的
    NSRange range = NSMakeRange(0, label.text.length);
    NSDictionary *dict = [attrib attributesAtIndex:0 effectiveRange:&range]; // 方法过期，自己加的
	CGSize labelSize = [label.text sizeWithAttributes:dict]; // 方法过期，自己加的
    
//	CGSize labelSize = [label.text sizeWithFont:label.font]; // 方法过期
    ...
    NSAttributedString *attribDetail = detailsLabel.attributedText; // 方法过期，自己加的
    NSRange rangeDetail = NSMakeRange(0, detailsLabel.text.length); // 方法过期，自己加的
    NSDictionary *dictDetail = [attribDetail attributesAtIndex:0 effectiveRange:&rangeDetail]; // 方法过期，自己加的
    CGSize detailsLabelSize = [detailsLabel.text boundingRectWithSize:maxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:dictDetail context:nil].size; // 方法过期，自己加的
    
//	CGSize detailsLabelSize = [detailsLabel.text sizeWithFont:detailsLabel.font constrainedToSize:maxSize lineBreakMode:detailsLabel.lineBreakMode]; // 方法过期
